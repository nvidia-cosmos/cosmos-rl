...
class PolicyStatusManager:
    ...
    def _publish_payload_transport_cleanup(
        ...
        """Publish per-transport cleanup for discarded rollouts.

        Each registered :class:`PayloadTransport` may stamp its rollout
        completions with a sentinel prefix.  When the controller
        discards rollouts as outdated, we group them by transport and
        delegate cleanup publishing to each backend so it can release
        any reserved resources (e.g. GPU buffers held by NCCL transfers).

        This is a no-op when no discarded rollouts carry a transport
        completion prefix.
        """
        kept_ids = {id(r) for r in filtered}

        # Group transfer ids by transport so each backend gets a single
        # batched cleanup invocation.
        per_transport: dict = {}
        for rollout in rollouts:
            if id(rollout) in kept_ids:
                continue
            completion = getattr(rollout, "completion", None)
            transport = PayloadTransportRegistry.active_for_completion(completion)
        ...
        for transport, transfer_ids in per_transport.items():
            try:
                ...
                if published > 0:
                    logger.debug(
                        f"[Controller] Published {transport.name} cleanup for "
                        f"{published}/{len(transfer_ids)} discarded transfers"
                    )
            except Exception as e:
                logger.debug(
                    f"[Controller] Transport {transport.name} cleanup failed: {e}"
                )
