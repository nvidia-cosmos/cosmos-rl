    ...
    def sync_all_states(
        ...
        for state_to_sync in model_state_dict:
            for dest_name in sorted(state_to_sync.keys()):
                ...
                if is_send:
                    logger.info(f"LMS: replica name: {self.replica_name}, rank: {self.global_rank}, send {dest_name} {local_view.shape} {local_view.dtype} {local_view.device}")
                    send_hook(local_view)
                else:
                    logger.info(f"LMS: replica name: {self.replica_name}, rank: {self.global_rank}, recv {dest_name} {local_view.shape} {local_view.dtype} {local_view.device}")
