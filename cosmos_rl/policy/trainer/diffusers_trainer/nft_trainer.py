...
class NFTTrainer(DiffusersTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.trainable_params = self.model.trainable_parameters
        self.model.transformer.set_adapter("ref")
        self.ref_trainable_params = self.model.trainable_parameters
        self.model.transformer.set_adapter("default")
        copy_weights(
        ...
        self.num_train_timesteps = (
            ...
            * self.config.train.train_policy.timesteps_fraction
        )
    ...
    def step_training(
        ...
        **kwargs,
        ...
        for i_mu in range(self.mu_iterations):
            logger.info(
                f"Mu iteration {i_mu + 1}/{self.mu_iterations} for training step {current_step}"
            )
            ...
            train_sample_batch = {k: v[perm] for k, v in packed_train_batch.items()}
            ...
            for j_idx, j_timestep_orig_idx in enumerate(
                ...
                advantages_clip = torch.clamp(
                    ...
                    self.config.policy.train_policy.advantage_low,
                    self.config.policy.train_policy.advantage_high,
                )

                # normalize advantage
                normalized_advantages_clip = (
                    advantages_clip / self.config.policy.train_policy.advantage_high
                ) / 2.0 + 0.5
                ...
                positive_prediction = (
                    self.config.policy.train_policy.kl_beta * forward_prediction
                    + (1 - self.config.policy.train_policy.kl_beta)
                    * old_prediction.detach()
                )
                implicit_negative_prediction = (
                    (1.0 + self.config.policy.train_policy.kl_beta)
                    * old_prediction.detach()
                    - self.config.policy.train_policy.kl_beta * forward_prediction
                )
                ...
                ori_policy_loss = (
                    r * positive_loss / self.config.policy.train_policy.kl_beta
                    + (1.0 - r)
                    * negative_loss
                    / self.config.policy.train_policy.kl_beta
                )
                policy_loss = (
                    ori_policy_loss * self.config.policy.train_policy.advantage_high
                ).mean()
                ...
                loss += self.config.policy.train_policy.kl_beta * torch.mean(
                    kl_div_loss
                )
