...
from cosmos_rl.utils.diffusion import callback, distributed
from cosmos_rl.utils.diffusion.ema import ema_scope
from cosmos_rl.utils.diffusion.model_loader import (
    create_model_from_consolidated_checkpoint_with_fsdp,
)
from cosmos_rl.utils.diffusion.utils import (
    to,
...
from cosmos_rl.utils.diffusion.checkpointer import (
    Checkpointer,
    DistributedCheckpointer,
)
from cosmos_rl.policy.trainer import Trainer
from cosmos_rl.utils.parallelism import ParallelDims
from cosmos_rl.policy.model.diffusion.models.v2v_model import Vid2VidModel
from cosmos_rl.policy.model.diffusion.models.t2v_model import InferenceInfos
from cosmos_rl.rollout.diffusion_rollout.diffusion_rollout import DiffusionRollout


# text encoder
from cosmos_rl.policy.model.diffusion.networks.vlm_qwen.processor import build_tokenizer
from cosmos_rl.policy.model.diffusion.networks.vlm_qwen.qwen_omni import QwenVLBaseModel

from cosmos_rl.utils.diffusion.checkpointer import (
    load_checkpoint,
)
from cosmos_rl.utils.diffusion.ref import (
    ref_scope,
)
from cosmos_rl.utils.diffusion.distributed import (
    is_tp_cp_pp_rank0,
)
from cosmos_rl.utils.diffusion.utils import (
...
from cosmos_rl.policy.config.diffusion import (
    EmbeddingConcatStrategy,
)

from cosmos_rl.utils.diffusion.context_parallel import (
    split_inputs_cp,
    cat_outputs_cp,
)


class CosmosVisionGenTrainer(Trainer):
    ...
    def trainer_init(self, config: CosmosVisionGenConfig, parallel_dims: ParallelDims):
        """
        Overide the trainer_init method to add the diffusion specific initialization.
        """
        self.config = (
        ...
        if isinstance(
            config.checkpoint.load_path, str
        ) and config.checkpoint.load_path.endswith(".pt"):
        ...
        self.rollout_runner = DiffusionRollout(config)
    ...
    def _training_step(
        ...
        if self.config.model.grpo.enabled:
            # Grpo training step
            return self.grpo_training_step(data_batch, iteration)
    ...
    def grpo_training_step(
        ...
        log-probability ratios for GRPO training instead of standard diffusion loss.
        ...
        grpo_inference_result = self.model.inference_infos.grpo_cached_trajectory[
            self.model.inference_infos.timesteps
        ]

        # Extract trajectory information
        noise_x = grpo_inference_result["noise_x"]  # x_t
        sigma_cur = grpo_inference_result["sigma"]  # σ_t
        sigma_next = grpo_inference_result["sigma_next"]  # σ_{t-1}
        mu_old = grpo_inference_result["mu_old"]  # reference mean μ_ref
        sample = grpo_inference_result["sample"]  # x_{t-1} (actual next sample)
        ...
        eta = min(
            self.model.grpo_solver_cfg.s_churn
            / (self.config.model.grpo.sample_steps + 1),
            math.sqrt(1.2) - 1,
        )

        std = (
            sigma_next
            * math.sqrt(eta**2 + 2 * eta)
            * self.model.grpo_solver_cfg.s_noise
        )
        ...
        if self.config.model.grpo.kl_beta > 0:
            kl_loss = (
                (grpo_inference_result["mu_ref"].to(mu_current.dtype) - mu_current)
                .pow(2)
                .mean()
            )
        else:
        ...
        advantages = [
            torch.clamp(
                r["advantage"],
                min=self.config.model.grpo.reward_config.adv_clip_min,
                max=self.config.model.grpo.reward_config.adv_clip_max,
            )
            for r in rewards.values()
        ]
        advantages = sum(advantages) / N_RM

        unclipped_loss = -advantages * ratio
        clipped_loss = (
            -torch.clamp(
                ratio,
                min=1 - self.config.model.grpo.clip_ratio,
                max=1 + self.config.model.grpo.clip_ratio,
            )
            * advantages
        )
        policy_loss = torch.maximum(unclipped_loss, clipped_loss).mean()

        loss = policy_loss + self.config.model.grpo.kl_beta * kl_loss
        ...
        output_batch = {
            ...
            "std": std.mean(),
            "eta": eta,
            "trajectory_step": grpo_inference_result["denoising_step"],
            "mse_loss": loss,
        ...
        output_batch["reward_instance"] = {
            k: r["mean_reward"].mean() for k, r in rewards.items()
        }
        ...
        output_batch["reward_mean"] = (
            sum([r["mean_reward"].mean() for r in rewards.values()]) / N_RM
        )
        ...
        if self.model.inference_infos.timesteps >= len(self.config.model.grpo.train_on):
        ...
        logger.info(
            f"iteration: {iteration}, denoising_step: {grpo_inference_result['denoising_step']}, loss: {loss.mean()}, advantage: {advantages.mean()}, ratio: {ratio.mean()}, kl_loss: {kl_loss.mean()}"
        )
    ...
    def training_step(
        ...
        if (
            ...
            and hasattr(model_ddp.config, "grpo")
            and model_ddp.config.grpo.enabled
        ):
            assert (
                model_ddp.config.grpo.sample_steps > 0
            ), "GRPO sample steps must be greater than 0"
            assert (
                len(model_ddp.config.grpo.train_on) > 0
            ), "GRPO train on must be set to a non-empty list"
            effective_grad_accum_iter *= len(model_ddp.config.grpo.train_on)
    ...
    def setup_text_encoder(self):
        assert self.config.model.text_encoder_class.startswith(
            "reason1"
        ), "Only reason1 online computation is supported"

        with torch.device("meta"):
            qwen_vl_processor = build_tokenizer(
                self.config.model.text_encoder_config.tokenizer_type
            )
            self.text_encoder = QwenVLBaseModel(
                self.config.model.text_encoder_config.encoder_model_config,
                qwen_vl_processor,
            )
        self.text_encoder.to_empty(device="cuda")
        with torch.no_grad():
            self.text_encoder.init_weights()
        load_checkpoint(
            self.text_encoder, self.config.model.text_encoder_config.ckpt_path
        )
    ...
    def compute_text_embeddings_online(
        ...
        for sample_idx in range(len(data_batch[self.input_caption_key])):
            conversations = [
                {
                    "role": "system",
                    "content": [
                ...
                },
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": data_batch[self.input_caption_key][sample_idx],
                        }
                    ],
                },
            ]
            tokenizer_output = self.text_encoder.tokenizer.apply_chat_template(
                conversations,
                tokenize=True,
            ...
            pad_id = self.text_encoder.tokenizer.pad_id
        ...
        with torch.no_grad():
            _, outputs_batch = self.text_encoder(input_ids_batch, {})
        ...
        if self.config.model.text_encoder_config.embedding_concat_strategy == str(
            ...
            text_embeddings_for_diffusion = torch.stack(normalized_hidden_states)
            text_embeddings_for_diffusion = text_embeddings_for_diffusion.mean(dim=0)
        elif self.config.model.text_encoder_config.embedding_concat_strategy == str(
            ...
            for i in range(0, len(normalized_hidden_states), n_layers_per_group):
                ...
                text_embeddings_for_diffusion.append(group_embedding)
            text_embeddings_for_diffusion = torch.cat(
                text_embeddings_for_diffusion, dim=-1
            )
        else:
            raise ValueError(
        ...
        return text_embeddings_for_diffusion
    ...
    def get_rollout_samples(self, data_batch: dict[str, torch.Tensor], iteration: int):
        ...
        """
        _, x0_B_C_T_H_W, _ = self.model.get_data_and_condition(data_batch)
        logger.info(
            f"[GRPO] Performing rollout inference at iteration {iteration}. Data shape to broadcast: {x0_B_C_T_H_W.shape}"
        )

        context = (
            nullcontext()
            if self.config.model.grpo.on_policy
            else ref_scope(
        ...
        logger.info(f"[GRPO] Using {context} for rollout inference")
        ...
        with torch.no_grad():
            with context:
                ...
                sample = self.rollout_runner.rollout_generation(
                    data_batch,
                    guidance=self.config.model.grpo.guidance,
                    num_steps=self.config.model.grpo.sample_steps,
                    seed=data_batch["seed"],
                ...
                logger.info("[GRPO] Start computing rewards ...")
                ...
                for k, v in self.model.reward_models.items():
                    ...
                    if valid:
                        try:
                            ...
                            logger.warning(
                                f"[GRPO] Reward model {k} failed: {e}. Ignore current rollout.",
                            )
                            valid = False
                    if not valid:
                        reward, bg_id = (
                ...
                logger.info("[GRPO] Finished computing rewards ...")
                torch.cuda.empty_cache()
        ...
        for i in range(len(trajs)):
            ...
            if self.model.grpo_solver_cfg.is_multi:
                trajs[i]["kwargs"] = {
        ...
        self.model.inference_infos.grpo_cached_trajectory = [
            w for i, w in enumerate(trajs) if i in self.config.model.grpo.train_on
        ]

        if (
            self.config.model.grpo.kl_beta > 0
        ):  # compute mu_ref here to reduec memory peak
            logger.info(
                f"[GRPO] Computing KL loss with reference model, kl_beta: {self.config.model.grpo.kl_beta}"
            )
            with ref_scope(
                model=self.model.net,
                ref_model=self.model.net_ref,
                ...
                for (
                    grpo_inference_result
                ) in self.model.inference_infos.grpo_cached_trajectory:
                    mu_ref = self.model.get_mu_from_model(
                        self.model.inference_infos.x0_fn,
                        grpo_inference_result["noise_x"],
                        grpo_inference_result["sigma"],
                        grpo_inference_result["sigma_next"],
                        **grpo_inference_result["kwargs"],
                    ).detach()
                    grpo_inference_result["mu_ref"] = mu_ref

        self.model.inference_infos.timesteps = 0
        torch.cuda.empty_cache()

        logger.info(
            f"[GRPO] Cached trajectory with {len(self.model.inference_infos.grpo_cached_trajectory)} steps at {self.config.model.grpo.train_on}, sample shape (with parallelism): {sample.shape}"
        )
