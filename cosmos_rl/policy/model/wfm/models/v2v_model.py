...
from cosmos_rl.policy.model.wfm.conditioner import Vid2VidCondition
from cosmos_rl.policy.model.wfm.conditioner.condition import T2VCondition
from cosmos_rl.policy.model.wfm.models.t2v_model import (
    WorldFoundationalModel as T2VModel,
)
...
class Vid2VidModel(T2VModel):
    ...
    ) -> Tuple[Tensor, Tensor, Vid2VidCondition]:
        ...
        if is_video_batch:
            if self.config.high_sigma_strategy == str(HighSigmaStrategy.UNIFORM80_2000):
                mask = (
                    ...
                    < self.config.high_sigma_ratio
                )
                new_sigma = (
                    torch.rand(sigma_B_1.shape, device=sigma_B_1.device).type_as(
                        sigma_B_1
                    )
                    * 1920
                    + 80
            ...
            elif self.config.high_sigma_strategy == str(
                HighSigmaStrategy.LOGUNIFORM200_100000
            ):
                mask = (
                    torch.rand(sigma_B_1.shape, device=sigma_B_1.device)
                    < self.config.high_sigma_ratio
                ...
                HighSigmaStrategy.BALANCED_TWO_HEADS_V1
            ):
                ...
                HighSigmaStrategy.HARDCODED_20steps
            ):
                if not hasattr(self, "hardcoded_20steps_sigma"):
                    from cosmos_rl.policy.model.wfm.sampler import get_rev_ts

                    hardcoded_20steps_sigma = get_rev_ts(
                        t_min=self.sde.sigma_min,
                        t_max=self.sde.sigma_max,
                        num_steps=20,
                        ts_order=7.0,
                    )
                    ...
                    self.hardcoded_20steps_sigma = torch.cat(
                        [
                            torch.tensor(
                                [100000.0], device=hardcoded_20steps_sigma.device
                            ),
                            hardcoded_20steps_sigma,
                        ],
                        dim=0,
                    )
                ...
                raise ValueError(
                    f"High sigma strategy {self.config.high_sigma_strategy} is not supported"
                )
    ...
    def get_x0_fn_from_batch(
        ...
        def x0_fn_NoCFG(noise_x: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
