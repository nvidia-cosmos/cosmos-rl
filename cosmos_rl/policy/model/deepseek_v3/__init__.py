...
class DeepseekV3MoEModel(BaseModel):
    ...
    def forward(
        ...
        **data_batch: Optional[torch.Tensor],
        ...
        logger.debug(f"[model] input ids shape: {input_ids.shape}, position_ids: {position_ids.shape}")
    ...
    def load_hf_weights(
        ...
        # TODO: avoid hacks based on directory names (e.g., checkpoing dir naming for DCP or _hf suffix for the HF format)
        dcp_checkpoint_path = os.path.join(DCP_CHECKPOINT_PATH_PREFIX, model_name_or_path.split("/")[-1].lower(), DCP_CHECKPOINT_PATH_SUFFIX)
        # if it is a huggingface model and no checkpoint exists, we need to load the weights from the safetensors files
        if len(model_name_or_path.split("/")) == 2 and (not os.path.exists(dcp_checkpoint_path) or len(os.listdir(dcp_checkpoint_path)) == 0):
            ...
            for f in safetensors_files:
                ...
                for name in weights_of_ckpt.keys():
                    ...
                    if ("down_proj" in name or "up_proj" in name or "gate_proj" in name or "self_attn.kv_a_proj_with_mqa" in name or "self_attn.kv_b_proj" in name or "self_attn.o_proj" in name or "self_attn.q_a_proj" in name or "self_attn.q_b_proj" in name) and "weight" in name:
                        inv_name = name + "_scale_inv"
                        inv_tensor = retry(safe_open)(
                            scale_inv_paths[inv_name], framework="pt", device=str(device)
                        ).get_tensor(inv_name)
                    ...
                    dest_name, shared_weight, expert_id = convert_weight_from_hf(
                        ...
                        model_type,
                        parallel_dims,
                        n_experts=deepseek_config.n_routed_experts,
                    )
                    ...
                    if expert_id is not None:
                        ...
                        n_local_experts = (
                            deepseek_config.n_routed_experts
                            // parallel_dims.tp
            ...
            mapped_state_dict = {
                k: v for k, v in self.state_dict().items() if not k.endswith("_extra_state")
            }
    ...
    def weight_sync_transforms(
        ...
        for local_name, param in self.named_parameters():
            ...
            assert hf_name not in transforms, f"Duplicate key found in transforms: {hf_name}"
