...
from cosmos_rl.utils.parallelism_registry import (
    ParallelismStrategyRole, register_parallelism_strategy, get_rollout_parallelism_strategy
)
from cosmos_rl.utils.logging import logger


# Pre-compile regex patterns for better performance
_LAYER_ATTN_NORM_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.self_attn\.(q_norm|k_norm|v_norm)\.(weight|bias)")
_LAYER_INPUT_LAYERNORM_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.input_layernorm\.(weight|bias)")
_LAYER_ATTN_PROJ_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.self_attn\.(q_proj|kv_b_proj|q_b_proj)\.(weight|bias)")
_LAYER_ATTN_KV_A_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.self_attn\.(kv_a_proj_with_mqa)\.(weight|bias)")
_LAYER_ATTN_O_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.self_attn\.(o_proj)\.(weight|bias)")
_LAYER_MLP_EXPERTS_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.mlp\.experts\.(\d+)\.(up_proj|gate_proj|down_proj)\.(weight|bias)")
_LAYER_MLP_SHARED_EXPERTS_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.mlp\.shared_experts\.(up_proj|gate_proj)\.(weight|bias)")
_LAYER_MLP_SHARED_EXPERTS_DOWN_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.mlp\.shared_experts\.down_proj\.(weight|bias)")
_LAYER_MLP_PROJ_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.mlp\.(up_proj|gate_proj)\.(weight|bias)")
_LAYER_MLP_DOWN_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.mlp\.down_proj\.(weight|bias)")
_LAYER_POST_ATTENTION_LAYERNORM_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.post_attention_layernorm\.(weight|bias)")
_LAYER_KV_A_LAYERNORM_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.self_attn\.kv_a_layernorm\.(weight|bias)")
_LAYER_Q_A_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.self_attn\.(q_a_layernorm|q_a_proj)\.(weight|bias)")
_LAYER_MLP_GATE_WEIGHT_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.mlp\.gate\.weight")
_LAYER_ROTARY_EMB_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.self_attn\.rotary_emb\.inv_freq")
_LAYER_MLP_GATE_BIAS_PATTERN = re.compile(r"model\.model\.layers\.(\d+)\.mlp\.gate\.e_score_correction_bias")
...
def map_key_from_hf(name: str, src_model_type: str) -> str:
    return name.replace("model.", "model.model.")
...
def convert_weight_from_hf(
    ...
    # Regex matching for complex patterns
        ...
        shard = tensor.tensor_split(tp_ep_size, dim=0)[tp_ep_rank]
    elif (match := _LAYER_MLP_SHARED_EXPERTS_DOWN_PATTERN.search(dest_name)) is not None:
        if dest_name.endswith(_BIAS_SUFFIX):
            shard = tensor
        else:
            shard = tensor.tensor_split(tp_ep_size, dim=-1)[tp_ep_rank]
        ...
        if dest_name.endswith(_BIAS_SUFFIX):
            ...
            shard = tensor.tensor_split(tp_ep_size, dim=-1)[tp_ep_rank]
    elif (match := _LAYER_POST_ATTENTION_LAYERNORM_PATTERN.search(dest_name)) is not None:
        shard = tensor
    ...
    if should_do_fsdp_sharding:
        if match := _LAYER_ATTN_KV_A_PATTERN.search(dest_name) is not None and (576 % dp_shard_size != 0):
            tensor_shard_size = 576 // dp_shard_size + 1
            if dp_shard_rank < (576 // tensor_shard_size):
                shard = shard[dp_shard_rank * tensor_shard_size:(dp_shard_rank + 1) * tensor_shard_size]
            else:
    ...
    expert_id = None
    if match := re.search(r"model\.model\.layers\.(\d+)\.mlp\.experts\.(\d+)\.(up_proj|gate_proj|down_proj)\.(weight|bias)", dest_name):
        expert_id = int(match.group(2))
        dest_name = dest_name.replace(f"experts.{expert_id}.", "experts.")
        dest_name = dest_name.replace(".weight", "s")
    
    if dest_name.startswith("lm_head"):
        dest_name = "model." + dest_name
...
class DeepseekV3MoEWeightMapper(WeightMapper):
    ...
    def policy_map_local_key_to_hf_key(self, name: str) -> str:
        ...
        param_name_map = {
            # mm_projector
            "mm_projector.projector.1.weight": "mlp1.0.weight",
            "mm_projector.projector.1.bias": "mlp1.0.bias",
            "mm_projector.projector.2.weight": "mlp1.1.weight",
            "mm_projector.projector.2.bias": "mlp1.1.bias",
            "mm_projector.projector.4.weight": "mlp1.3.weight",
            "mm_projector.projector.4.bias": "mlp1.3.bias",
            # language_model
            # "model.lm_head.weight": "language_model.lm_head.weight",
        }
        if name in param_name_map:
            name = param_name_map[name]
        ...
        for local_prefix, hf_prefix in prefix_map.items():
            if name.startswith(local_prefix):
                name = hf_prefix + name[len(local_prefix) :]
                break

        # 3. substring match and replace
    ...
    def get_rollout_parallelism_strategy(self):
        return [get_rollout_parallelism_strategy("internvl_chat")]


@register_parallelism_strategy("internvl_chat", role=ParallelismStrategyRole.ROLLOUT)
...
#) -> Tuple[Dict[str, int], list, Dict[int, list]]:
    ...
    logger.debug(f"Rollout parallelism mapping: Dest_name: {dest_name}, tp_dim: {tp_dim}")
