...
def remap_model_state_for_deepep(
    ...
    for key, v in state_dict.items():
        ...
        if match:
            ...
            logger.info(f"Remapping {key} to {prefix}model.layers.{layer_num}.mlp.experts.xxx_projs")

            v_1, v_2 = torch.chunk(v, 2, -1)
            new_state_dict[f"{prefix}model.layers.{layer_num}.mlp.experts.gate_projs"] = v_1.transpose(1, 2)
            new_state_dict[f"{prefix}model.layers.{layer_num}.mlp.experts.up_projs"] = v_2.transpose(1, 2)
