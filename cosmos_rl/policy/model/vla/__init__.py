...
class OpenVLA(BaseModel):
    ...
    def __init__(self, model_name_or_path: str, hf_config: AutoConfig):
        ...
        with torch.device("cpu"):
            self.processor = PrismaticProcessor.from_pretrained(model_name_or_path, trust_remote_code=True)
            self.tokenizer = self.processor.tokenizer

        # Create model with specified device (use "meta" for fast initialization)
        with torch.device('cuda'):
            self.model = OpenVLAForActionPrediction(self.hf_config).to(hf_config.torch_dtype)
    ...
    def preprocess_hf_config(cls, cosmos_config: CosmosConfig):
        ...
        try:
            ...
            dataset_statistics_path = os.path.join(local_model_path, "dataset_statistics.json")
    ...
    def _replace_rope_modules_float32(self):
        ...
        try:
            ...
            if hasattr(self.model.language_model.model, 'rotary_emb'):
                ...
                logger.debug(f"Replaced top-level rotary_emb: {old_dtype} → {new_rope.inv_freq.dtype}")
                replaced_count += 1
