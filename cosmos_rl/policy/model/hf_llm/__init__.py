...
class RotaryEmbedding(nn.Module):
    def __init__(self, args: HFLLMArgs, device=None):
        super().__init__()
        self.args = args
...
class HFLLMBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, model_args: HFLLMArgs):
        super().__init__()
        self.n_heads = model_args.n_heads
    ...
    def forward(
        ...
        """
        Perform a forward pass through the HFLLMBlock.
...
class HFLLMModel(BaseModel):
    ...
    def __init__(self, model_args: HFLLMArgs):
        ...
        self.src_model_type = model_args.hf_config.model_type
        ...
        for layer_id in range(model_args.n_layers):
            self.layers[str(layer_id)] = HFLLMBlock(layer_id, model_args)
    ...
    @classmethod
    def from_model_args(cls, model_args: HFLLMArgs) -> "HFLLMModel":
        """
    ...
    def from_pretrained(
        ...
        q_k_norm_enabled = hf_config.model_type in ["qwen3"]
        ...
        model = cls.from_model_args(
            HFLLMArgs(
                ...
                q_k_norm_enabled=q_k_norm_enabled,
                norm_type=norm_type,
                rope_type=rope_type,
                biases=bias_list,
                hf_config=hf_config,
