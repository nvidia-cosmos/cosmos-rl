...
class BaseModel(torch.nn.Module, ABC):
    ...
    def weight_sync_transforms(self) -> List[Tuple[str, Union[torch.Tensor, Callable]]]:
        ...
        for name, param in self.named_parameters():
            dims_rank_info, dims_map = extract_infomation_from_DTensor(
                param, name, self.get_uneven_shard_params(name)
            )
    ...
    def get_uneven_shard_params(
        self, param_name: str
    ) -> Optional[Tuple[torch.Size, List[int]]]:
        """
        ...
        """
        if (
            not hasattr(self, "_uneven_shard_params")
            or param_name not in self._uneven_shard_params
        ):
            return None
    ...
    def get_local_shard(
        ...
        if (
            hasattr(self, "_uneven_shard_params")
            and param_name in self._uneven_shard_params
        ):
