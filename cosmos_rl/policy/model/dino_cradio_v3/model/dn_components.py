...
def prepare_for_cdn(dn_args, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    if training:
        ...
        # Create padding tensors with dtypes matching their eventual sources to avoid dtype mismatch on index_put
        padding_label = torch.zeros(
            pad_size, hidden_dim, dtype=input_label_embed.dtype, device="cuda"
        )
        padding_bbox = torch.zeros(
            pad_size, 4, dtype=input_bbox_embed.dtype, device="cuda"
        )
