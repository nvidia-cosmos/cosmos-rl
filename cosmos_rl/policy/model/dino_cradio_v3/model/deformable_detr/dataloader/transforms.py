...
def interpolate(
    inputs, size=None, scale_factor=None, mode="nearest", align_corners=None
):
    """
...
def build_transforms(augmentation_config, subtask_config=None, dataset_mode="train"):
    ...
    if dataset_mode == "train":
                    ...
                    RandomSelect(
                        ...
                        Compose(
                            [
                                ...
                                RandomSizeCrop(
                                    train_random_crop_min, train_random_crop_max
                                ),
                                RandomResize(scales, max_size=random_resize_max_size),
                            ]
                        ),
        ...
        if (
            subtask_config
            and subtask_config["input_width"]
            and subtask_config["input_height"]
        ):
            transforms = Compose(
                [
                    FixedResize(
                        (subtask_config["input_width"], subtask_config["input_height"])
                    ),
                    normalize,
                ]
            )
        ...
        raise ValueError(
            "There are only train, val, eval, and infer options in dataset_mode."
        )
...
class ResizeScale(object):
    ...
    def _get_resize(self, image: np.ndarray, target: dict, scale: float):
        ...
        output_size = np.round(
            np.multiply((image_width, image_height), output_scale)
        ).astype(int)
