...
# Register tokenizer mapping - Pi05/Pi0 use GemmaTokenizer since they're based on PaliGemma
try:
    from transformers import GemmaTokenizer, GemmaTokenizerFast
    # Register both slow and fast tokenizer variants
    AutoTokenizer.register(Pi05Config, slow_tokenizer_class=GemmaTokenizer, fast_tokenizer_class=GemmaTokenizerFast)
    logger.info(f"Pi05 tokenizer registration succeeded")
except (ValueError, ImportError) as e:
    # already registered or tokenizer not available
    logger.debug(f"Pi05 tokenizer registration skipped: {e}")
