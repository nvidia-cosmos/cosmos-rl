...
from cosmos_rl.policy.model.pi05.model_utils import gemma as get_config
from cosmos_rl.policy.model.pi05.model_utils import PaliGemmaWithExpertModel
from cosmos_rl.policy.model.pi05.model_utils import preprocessing_pytorch as _preprocessing
...
class PI05(BaseModel):
    ...
    def get_position_ids(self, **kwargs):
        raise NotImplementedError("PI05 does not use token-based inputs; CP is not supported")
    ...
    def load_hf_weights(
        ...
        if safetensors_files:
            ...
            for fname in pt_files:
                state_dict.update(torch.load(os.path.join(model_path, fname), map_location="cpu", weights_only=True))

        # Load with strict=False to handle any key mismatches
        missing, unexpected = self.load_state_dict(state_dict, strict=False)
        
        if missing:
            logger.warning(f"PI05 weight load: {len(missing)} missing keys. First 10: {missing[:10]}")
        if unexpected:
            logger.info(f"PI05 weight load: {len(unexpected)} unexpected keys (ignored)")
        logger.info(f"PI05 weight load done. loaded={len(state_dict) - len(unexpected)} weights")
    ...
    *,
    ...
    for key in image_keys:
        ...
        if image.shape[1:3] != image_resolution:
            logging.info(f"Resizing image {key} from {image.shape[1:3]} to {image_resolution}")
