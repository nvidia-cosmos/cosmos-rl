...
class SanaModel(DiffuserModel):
    ...
    def visual_embedding(self, input_visual_list, height=None, width=None, device='cuda'):
        ...
        if self.is_video:
            with torch.no_grad():
                visual_embedding = self.vae.encode(input_samples.to(self.vae.dtype), return_dict=False)[0]
            ...
            with torch.no_grad():
                ...
                visual_embedding = (self.vae.encode(input_samples.to(self.vae.dtype), return_dict=True).latent) * scaling_factor
