...
from cosmos_rl.policy.config.diffusion import CosmosVisionGenConfig as Config
from cosmos_rl.policy.trainer.diffusion_trainer import CosmosVisionGenTrainer
from cosmos_rl.utils.diffusion.io import dump_dict_to_file
from cosmos_rl.utils.diffusion.io.cred_env_parser import CRED_ENVS, CRED_ENVS_DICT
from cosmos_rl.utils.diffusion.io.easy_io import easy_io
...
def init_s3(config: Config):
    ...
    if str(CRED_ENVS):
        ...
        s3_training_path = os.path.join(CACHE_DIR, "credentials", "s3_training.secret")
        ...
        if config.model.text_encoder_config is not None:
            config.model.text_encoder_config.encoder_model_config.s3_credential_path = (
                pbss_dir_path
            )
...
class DiffusionPolicyWorker(WorkerBase):
    """
    Diffusion RL is now in experimental stage.
    """
    ...
    def worker_init(self, **kwargs):
        ...
        if os.environ.get("DIFFUSION_REWARD_TOKEN", ""):
            self.config.model.grpo.reward_config.remote_reward.token = os.environ.get(
                "DIFFUSION_REWARD_TOKEN", ""
            )
            logger.info("Using token from environment variable")
        if os.environ.get("DIFFUSION_REWARD_ENQUEUE_URL", ""):
            self.config.model.grpo.reward_config.remote_reward.enqueue_url = (
                os.environ.get("DIFFUSION_REWARD_ENQUEUE_URL", "")
            )
            logger.info("Using enqueue_url from environment variable")
        if os.environ.get("DIFFUSION_REWARD_FETCH_URL", ""):
            self.config.model.grpo.reward_config.remote_reward.fetch_url = (
                os.environ.get("DIFFUSION_REWARD_FETCH_URL", "")
            )
    ...
    def build_runner(self, **kwargs):
        # For diffusion, parallel_dims is instantiated in the trainer now.
