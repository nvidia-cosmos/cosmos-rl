...
def test_injection_and_freeze(seed):
    model = TinyBlock()
    cfg = LoraConfig(
...
def test_forward_equivalence_merge_unmerge(seed):
    x = torch.randn(4, 32)
    model = TinyBlock()
    cfg = LoraConfig(
...
def test_grads_only_on_lora(seed):
    x = torch.randn(8, 32)
    y = torch.randn(8, 16)
    ...
    old_base = {
        n: p.clone().detach() for n, p in model.named_parameters() if "lora_" not in n
    }
    old_lora = {
    ...
    with torch.no_grad():
        ...
        for n, p in model.named_parameters():
            if "lora_" in n:
                assert not torch.equal(p, old_lora[n]), f"LoRA param {n} did not change"
...
def test_save_and_load_lora_only(seed):
    model = TinyBlock()
    cfg = LoraConfig(
        r=4, lora_alpha=8, lora_dropout=0.0, target_modules=["q_proj", "v_proj"]
    )
