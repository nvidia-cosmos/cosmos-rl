...
def log_tao_status(data: Dict[str, Any], step: int, component_name: str = "Cosmos-RL Training", max_steps: int = None):
    ...
    try:
        ...
        # Calculate time estimates (simplified - could be enhanced with actual timing)
        estimated_time_per_step = data.get('train/iteration_time', 1.0)  # Use actual iteration time if available
        remaining_steps = (max_steps - step) if max_steps and max_steps > step else 0
        eta_seconds = remaining_steps * estimated_time_per_step
        
        tao_data = {
            'component': component_name,
            'step': step,
            'max_step': max_steps if max_steps else step + 1000,  # Default if not provided
            'time_per_step': str(timedelta(seconds=estimated_time_per_step)),
            'eta': str(timedelta(seconds=eta_seconds))
        }
        
        # Create summary message based on available metrics
        if 'train/loss_avg' in data:
            message = f"Training step {step} - Loss: {data['train/loss_avg']:.6f}"
        elif 'val/loss' in data:
            message = f"Validation step {step} - Loss: {data['val/loss']:.6f}"
        else:
            message = f"Training loop in progress"
        ...
        logger.debug(f"TAO status logged for {component_name} step {step}")
        
    except Exception as e:
        import traceback
        logger.warning(traceback.format_exc())
        logger.warning(f"TAO status logging failed for step {step} with exception: {e}")
