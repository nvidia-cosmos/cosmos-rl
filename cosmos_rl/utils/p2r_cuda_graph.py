...
class P2RSendCudaGraphManager:
    ...
    def __init__(
        ...
        self._buffer_storages: Dict[bool, torch.Tensor] = {}
    ...
    def build(
        ...
        for insts_group in self._insts:
            for insts_for_per_param in insts_group.param_instructions:
                ...
                for inst in insts_for_per_param.instructions:
                    ...
                    raw_slots.append((dest_name, slice_strategy, view, r_rank, needs_fill))
        ...
        for slot in raw_slots:
            ...
            if cur_chunk and chunk_limit_bytes > 0 and cur_bytes + slot_bytes > chunk_limit_bytes:
        ...
        # Chunks replay sequentially — share one storage sized to the largest
        # chunk's needs_fill footprint (offset resets to 0 per chunk).
        max_chunk_numel = max(
            (sum(v.numel() for _, _, v, _, nf in chunk if nf) for chunk in raw_chunks),
            default=0,
        )
        shared_storage = torch.empty(
            max_chunk_numel, dtype=transfer_dtype, device=torch.cuda.current_device(),
        )
        self._buffer_storages[trainable_only] = shared_storage
        ...
        for raw_chunk in raw_chunks:
            ...
            for dest_name, ss, view, r_rank, needs_fill in raw_chunk:
                if needs_fill:
                    numel = view.numel()
                    send_buffer = shared_storage.narrow(0, offset, numel).view(view.shape)
                    offset += numel
                else:
                    send_buffer = view
        ...
        total_mb = sum(s[2].numel() * s[2].element_size() for s in all_slots) / (1024 * 1024)
        shared_mb = shared_storage.numel() * shared_storage.element_size() / (1024 * 1024)
        logger.info(
    ...
    def fill_and_replay(
        ...
        return P2RGraphStats(total_bytes, transferred, 0)
...
class P2RRecvCudaGraphManager:
    ...
    def build(self, base_mesh_key: str, trainable_only: bool) -> None:
        ...
        for insts_group in self._insts:
            for insts_for_per_param in insts_group.param_instructions:
                ...
                for inst in insts_for_per_param.instructions:
                    ...
                    raw_slots.append(
                        (inst_dest_name, slice_strategy, None, p_rank, is_inplace, underlying_view)
                    )
        ...
        for slot in raw_slots:
            ...
            slot_bytes = target_view.numel() * target_view.element_size() if not is_inplace else 0
            if cur_chunk and chunk_limit_bytes > 0 and cur_bytes + slot_bytes > chunk_limit_bytes:
                raw_chunks.append(cur_chunk)
                cur_chunk = []
                cur_bytes = 0
        ...
        # Shared storage sized to the largest chunk's not-inplace footprint.
        max_chunk_numel = max(
            (sum(tv.numel() for _, _, _, _, ip, tv in chunk if not ip) for chunk in raw_chunks),
            default=0,
        )
        shared_storage = torch.empty(
            max_chunk_numel, dtype=target_dtype, device=torch.cuda.current_device(),
        )
        self._buffer_storages[trainable_only] = shared_storage
        ...
        for raw_chunk in raw_chunks:
            ...
            for pname, ss, _, p_rank, is_inplace, target_view in raw_chunk:
                if is_inplace:
                    ...
                    recv_buffer = shared_storage.narrow(0, offset, numel).view(target_view.shape)
                    offset += numel
        ...
        for chunk_slots in chunks:
            ...
            graph_chunks.append(
                (graph, [(recv_buf, p_rank) for _, _, recv_buf, p_rank, _, _ in chunk_slots])
            )

        self._graphs[trainable_only] = (graph_chunks, all_post_ops, all_check_info)
    ...
    def replay(
        ...
        """
        graph_chunks, all_post_ops, all_check_info = self._graphs[trainable_only]
        ...
        if do_weight_sync_check:
            target_dtype = self._transfer_dtype
            for (target_view, _, pname), cloned in zip(all_check_info, cloned_check_tensors):
        ...
        transferred = sum(len(chunk_recv_ops) for _, chunk_recv_ops in graph_chunks)
        return P2RGraphStats(total_bytes, transferred, 0)
