...
def extract_infomation_from_DTensor(param: torch.Tensor, name: str, splited_dims: List[List[int]] = None) -> Tuple[Dict[int, Dict[str, Any]], Dict[str, int]]:
    """
    ...
    if isinstance(param, torch.distributed.tensor.DTensor):
        ...
        if splited_dims is not None:
            updated_dims_rank_info = {}
            handled_dims = set()
            dim_idx = 0
            for idx, dim in dims_rank_info.items():
                if idx in handled_dims:
                    continue
                for split_dim in splited_dims:
                    if idx in split_dim:
                        dims_rank = dims_rank_info[idx]
                        assert split_dim[0] == idx, f"Expected the first dimension in split_dim to be {idx}, but got {split_dim[0]}."
                        for cnt in range(1, len(split_dim)):
                            dims_rank = merge_rank(dims_rank, dims_rank_info[split_dim[cnt]] if split_dim[cnt] in dims_rank_info else no_shard_dims_rank_info[split_dim[cnt]])
                        updated_dims_rank_info[dim_idx] = dims_rank.__dict__
                        dim_idx += 1
                        handled_dims.update(split_dim)
            dims_rank_info = updated_dims_rank_info
        else:
            dims_rank_info = {idx: dim_info.__dict__ for idx, dim_info in dims_rank_info.items()}
                
    return dims_rank_info, dims_map

def get_local_weight_shard_with_DTensor(target: torch.Tensor, dest_name: str, tensor: torch.Tensor, splited_dims: List[List[int]] = None, mesh_dims: List[str] = ["tp", "dp_shard"]):
    """
    ...
    """
    dims_rank_info, dims_map = extract_infomation_from_DTensor(target, dest_name, splited_dims)
    dims_rank_info = {int(k): DimSliceInfo.from_dict(v) for k, v in dims_rank_info.items()}
    for mdim in mesh_dims:
        if mdim in dims_map:
            tp_dim = dims_map[mdim]
            dims_rank_info[tp_dim].offset = min(tensor.shape[tp_dim], dims_rank_info[tp_dim].offset)
            dims_rank_info[tp_dim].length = min(tensor.shape[tp_dim] - dims_rank_info[tp_dim].offset, dims_rank_info[tp_dim].length)
            dims_rank_info[tp_dim].total_size = min(tensor.shape[tp_dim], dims_rank_info[tp_dim].total_size)
...
def merge_continuous_dims(tensor: torch.Tensor, dims_list: List[List[int]]) -> torch.Tensor:
    """
