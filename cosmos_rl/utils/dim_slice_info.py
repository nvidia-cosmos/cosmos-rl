...
def extract_infomation_from_DTensor(
    param: torch.Tensor, name: str, splited_dims: List[List[int]] = None
    ...
    if isinstance(param, torch.distributed.tensor.DTensor):
        ...
        if splited_dims is not None:
            ...
            for idx, dim in dims_rank_info.items():
                ...
                for split_dim in splited_dims:
                    if idx in split_dim:
                        ...
                        assert (
                            split_dim[0] == idx
                        ), f"Expected the first dimension in split_dim to be {idx}, but got {split_dim[0]}."
                        for cnt in range(1, len(split_dim)):
                            dims_rank = merge_rank(
                                dims_rank,
                                dims_rank_info[split_dim[cnt]]
                                if split_dim[cnt] in dims_rank_info
                                else no_shard_dims_rank_info[split_dim[cnt]],
                            )
            ...
            dims_rank_info = {
                idx: dim_info.__dict__ for idx, dim_info in dims_rank_info.items()
            }
