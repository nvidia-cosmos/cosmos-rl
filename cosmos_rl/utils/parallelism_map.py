...
from cosmos_rl.utils.dim_slice_info import tensor_overlap_info_at_dim, merge_rank, get_unified_rank_info, relative_rank, rank_overlap
...
class ParallelTopoMapper:
    ...
    def parallelism_info_for_dtensor_params(self) -> None:
        ...
        for name, param in self.underlying_model.named_parameters(
            ...
            dims_rank_info, dims_map = extract_infomation_from_DTensor(param, name, self.underlying_model.get_uneven_shard_params(name))
            logger.info(
                f"===== [Policy] Processing parameter {name} with global shape {global_shape} and dims_rank_info {dims_rank_info}  {dims_map} in parallelism_info_for_dtensor_params."
            )
