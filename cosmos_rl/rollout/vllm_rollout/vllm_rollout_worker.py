...
class vLLMRolloutWorker(RolloutWorkerBase):
    ...
    def request_new_prompts(self, batch_size: int, prompt_queue: Queue, **kwargs):
        ...
        if self.global_rank == 0:
            total_prompts = (
                len(prompts_and_is_end[0]) if prompts_and_is_end[0] is not None else 0
            )
            if total_prompts > 0:
                logger.info(
                    f"[Rollout] Broadcasting prompts and is_end to all ranks: {prompts_and_is_end[1]} with {total_prompts} prompts."
                )
