...
# Compatibility shim for payload schema types when multi-turn schema is absent
HAS_PAYLOAD: bool = True
try:
    from cosmos_rl.dispatcher.data.schema import (
        RLPayload as NewRLPayload,
        IdxAndRLPayload as NewIdxAndRLPayload,
        ConversationType as NewConversationType,
    )
except Exception:
    HAS_PAYLOAD = False
    NewRLPayload = Any  # type: ignore
    NewIdxAndRLPayload = Any  # type: ignore
    NewConversationType = Any  # type: ignore
try:
    from cosmos_rl.rollout.schema import RolloutResult as NewRolloutResult
except Exception:
    NewRolloutResult = Any  # type: ignore
...
class vLLMRolloutWorker(RolloutWorkerBase):
    ...
    def do_validation(self):
        ...
        validation_payloads: List[Any] = []
        validation_completions: List[List[str]] = []
        # Do validation here
        while True:
            ...
            if not validation_queue.empty():
                prompt_id_and_payload_list: List[Any] = validation_queue.get()
                # Support (idx, payload) and (idx, payload, weight_version)
                pairs: List[tuple[int, Any]] = [
                    (item[0], item[1]) for item in prompt_id_and_payload_list
                ]
                rollout_results: List[List[str]] = self.rollout.rollout_generation(
                    prompt_id_and_payload_list=pairs,
                    stream=self.inference_stream,
                    data_packer=self.val_data_packer,
                    sampling_params=self.val_sampling_params,
                )
                if rollout_results:
                    prompt_idxs.extend([item[0] for item in prompt_id_and_payload_list])
                    validation_payloads.extend(
                        [item[1] for item in prompt_id_and_payload_list]
                    )
                    validation_completions.extend(rollout_results)
        ...
        if should_report:
            response = ValidationReportRequest(
                ...
                completions=validation_completions,
                is_end=True,
            )
            try:
                make_request_with_retry(
        ...
        if prompts is not None:
            if HAS_PAYLOAD:
                ...
                for prompt in prompts:
                    idx = prompt[0]
                    payload = prompt[1]
                    weight_version = prompt[2] if len(prompt) > 2 else None
                    if isinstance(payload, dict):
                        try:
                            payload = NewRLPayload.model_validate(payload)
                        except Exception:
                            pass
                    if weight_version is not None:
                        new_list.append((idx, payload, weight_version))
                    else:
                        new_list.append((idx, payload))
                prompt_queue.put(new_list)
            else:
