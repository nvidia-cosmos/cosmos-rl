...
def parallelize(
    ...
    if parallel_dims.tp_enabled:
        ...
        try:
            ...
            raise ImportError(
                "deep_ep is not installed. Please install it first to enable EP support."
            )
        ...
        language_model_config = (
            model.hf_config.text_config
            if "text_config" in model.hf_config
            else model.hf_config
        )
        gate_training_enabled = config.custom.get(
...
def apply_fsdp(
    ...
    if model.vision_model is not None:
        ...
        # multi_modal_projector is a submodule of vision model, apply fsdp to this module first
