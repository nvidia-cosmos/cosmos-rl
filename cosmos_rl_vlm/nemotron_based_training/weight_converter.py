...
def fsdp_sharding(
    shard: torch.Tensor, dp_shard_rank: int, dp_shard_size: int
) -> torch.Tensor:
...
def convert_weight_from_hf(
    ...
    language_model_config = (
        hf_config.text_config if "text_config" in hf_config else hf_config
    )
    ...
    if ep_size > 1 and "mixer.experts." in name:
        ...
        if (
            ...
            if belongs_to_current_ep and belongs_to_current_dp_shard:
                ...
                def transform_name_fn(
                    model_state_dict: Dict[str, torch.Tensor], name: str
                    ...
                    suffix = suffix.replace(
                        "up_proj.weight", "gate_and_up_projs"
                    ).replace("down_proj.weight", "down_projs")

                    local_expert_idx = int(match.group(2)) % (
                        n_experts // (ep_size * dp_shard_size)
                    )
        ...
        flattened_dp_group = parallel_dims.mesh[tuple(dp_group_names)]._flatten(
