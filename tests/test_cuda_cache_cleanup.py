...
def test_suppression_is_process_wide_sticky_and_does_not_synchronize(
    ...
    cuda_cache.suppress_opportunistic_cuda_cache_cleanup()
    cuda_cache.suppress_opportunistic_cuda_cache_cleanup()
    results = []
    threads = [
        threading.Thread(
            target=lambda: results.append(cuda_cache.maybe_empty_cuda_cache())
        )
        for _ in range(8)
    ]
...
def test_transport_start_waits_for_preexisting_flush(policy, monkeypatch):
    ...
    monkeypatch.setattr(cuda_cache.torch.cuda, "empty_cache", flush)
    flushing = threading.Thread(target=cuda_cache.maybe_empty_cuda_cache)
    flushing.start()
    ...
    assert not cuda_cache.maybe_empty_cuda_cache()
...
def test_native_setup_suppresses_before_partial_acquisition(
    ...
    monkeypatch.setattr(
        cuda_cache, "suppress_opportunistic_cuda_cache_cleanup", suppress
    )
    if backend == "nccl":
        if kind == "strategy":
...
def test_suppression_does_not_intercept_intentional_release(policy, monkeypatch):
    flush = Mock()
    monkeypatch.setattr(cuda_cache.torch.cuda, "empty_cache", flush)
    cuda_cache.suppress_opportunistic_cuda_cache_cleanup()
    assert not cuda_cache.maybe_empty_cuda_cache()
    # Represents a caller-owned, established idle boundary. The optional policy
    # neither intercepts this call nor certifies that a real boundary is safe.
    cuda_cache.torch.cuda.empty_cache()
    flush.assert_called_once()
    assert not cuda_cache.maybe_empty_cuda_cache()
...
@pytest.mark.parametrize(
    ...
    [
        "utils/checkpoint.py",
        "policy/trainer/llm_trainer/llm_trainer.py",
        "simulators/robotwin/venv.py",
    ],
)
def test_existing_memory_release_sites_are_not_reclassified(relative_path):
    ...
    assert "torch.cuda.empty_cache" in calls
    assert not any("maybe_empty_cuda_cache" in call for call in calls)
