...
def _decode_video_torchcodec(blob: bytes, ele: dict,
    ...
    sample_fps = nframes / max(orig_total_frames, 1) * video_fps
...
class CustomWebDatasetDataset(Dataset):
    ...
    def setup_wds_dataset(self):
        ...
        custom = self.cosmos_config.custom if hasattr(self.cosmos_config, "custom") and self.cosmos_config.custom is not None else {}
        wds_pool_shuffle_seed = custom.get("wds_pool_shuffle_seed", 42)
        wds_sharding_seed = custom.get("wds_sharding_seed", 12345)
...
if __name__ == "__main__":
    ...
    if "qwen" in vision_config.get("model_type", "").lower() or "siglip2" in vision_config.get("model_type", "").lower():
