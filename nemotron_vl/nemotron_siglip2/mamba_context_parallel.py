...
from cosmos_rl.utils.util import rank0_print
...
class MambaContextParallel:
    ...
    def __init__(
        ...
        self.cp_rank = self.cp_group.rank() if self.cp_group is not None else 0
    ...
    def pre_conv_ssm(
        ...
        # input_: [B, S // cp_size, projection_size]
        if self.cp_size == 1:
            return input_
        ...
        # [b, l_global//cp, d_inner] -> [b, l_global, d_inner//cp]
    ...
    def get_rmsnorm_weight(self) -> torch.Tensor:
        """Returns a slice of rmsnorm_weight relevant to the current context parallel rank"""
        return self._slice_vector_param(self.rmsnorm_weight_cp1, has_hdim=True) # Using has_hdim=True because rmsnorm_weight is a vector parameter
...
def _all_to_all_cp2hp(
    ...
    output = output.reshape(b_in, s_in * world_size, h_out)
    return output
...
def _all_to_all_hp2cp(
    ...
    b_in, s_in, h_in = input_.shape
    # Squash the first two dimensions -> [b*s, h]
    input_ = input_.reshape(-1, h_in)
    # Swap chunks of dim0 across the cp ranks
    ...
    split_tensors = torch.split(input_exchanged, split_size_or_sections=b_in * s_out, dim=0)
    # Concat the chunks along the h dimension
    output = torch.cat(split_tensors, dim=-1)
    # Recover the s and b dimensions
    output = output.reshape(b_in, s_out, h_in * world_size)
    return output
