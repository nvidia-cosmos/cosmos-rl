...
def approx_max_pixel(messages, max_seq_len):
    # text token length approximation
    text_token_length = int(len(orjson.dumps(messages).decode('utf-8')) // 3.0)
    max_pixels = (max_seq_len - text_token_length) * 0.9 * 32 * 32
    if max_pixels < 128 * 32 * 32:
        return None
    return max_pixels

def check_token_length(messages, max_seq_len):
    # text token length approximation
    text_token_length = int(len(orjson.dumps(messages).decode('utf-8')) // 3.0)
    max_pixels = (max_seq_len - text_token_length) * 0.9 * 32 * 32
    if max_pixels < 128 * 32 * 32:
...
def modify_messages(messages, max_seq_len = None):
    if not check_token_length(messages, max_seq_len):
        return None
        ...
        for file_name in jsonl_files:
            ...
            with open(os.path.join(data_path, file_name)) as f:
                for line in f:
                    line = line.strip()
                    if line:
                        data_dict = orjson.loads(line)
                        if 'image_meta' not in data_dict:
                            continue
                        modified_messages = modify_messages(data_dict['messages'], config.policy.model_max_length)
                        if modified_messages is not None:
                            self.data_list.append(modified_messages)
